...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - your song **%s** is Tocando em %s!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = 'Tocando em %s: **%s**' % (
    ...
    async def on_ready(self):
        ...
        print("  Tocando agora @mentions: " + ['Disabled', 'Enabled'][self.config.now_playing_mentions])
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                "You have reached your Na fila song limit (%s)" % permissions.max_songs, expire_in=30
            )
        ...
        if 'entries' in info:
            ...
            reply_text = "Na fila **%s** músicas para serem tocadas. Posição na fila: %s"
            ...
            reply_text = "Na fila **%s** Posição na fila: %s"
            btext = entry.title

        if position == 1 and player.is_stopped:
            position = 'Tocando agora!'
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        return Response("Na fila {} músicas para serem tocadas {} segundos".format(
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Tocando agora: **%s** adicionado por **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = "Tocando agora: **%s** %s\n" % (player.current_entry.title, prog_str)

            self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, np_text)
            await self._manual_delete_check(message)
        else:
            return Response(
                'Não existem músicas na fila! Coloque alguma usando {}play.'.format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Tocando agora: **%s** adicionado por **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append("Tocando agora: **%s** %s\n" % (player.current_entry.title, prog_str))

        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = '`{}.` **{}** adicionado por **{}**'.format(i, item.title, item.meta['author'].name).strip()
            else:
        ...
        if not lines:
            lines.append(
                'Não existem músicas na fila! Coloque alguma com o comando {}play.'.format(self.config.command_prefix))
